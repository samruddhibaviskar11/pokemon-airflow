...
def save_result():
    clients3 = boto3.client('s3', region_name='ap-south-1', aws_access_key_id='AKIAUSSWOWAPEUEK7OJR', aws_secret_access_key='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')
    print("hello")
    for i, filename in enumerate(os.listdir(process_dir)):
        predict_label,img=tuple(filename.split("_",1))
        result = clients3.upload_file(os.path.join(process_dir,filename), "pokemon.sourcebucket", predict_label+"/"+img)
